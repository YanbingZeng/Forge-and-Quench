...
from forge_and_quench.models.modeling_forge_and_quench import FaQAttnProcessor
...
class LongCatImagePipeline(DiffusionPipeline, FromSingleFileMixin):
    ...
    def prepare_for_quench(self, quench_model_path):
        # step1: get attn processors
        processors = {}
        ...
        # step2: set faq attn processors
        for name in processors.keys():
            if name.startswith("transformer_blocks."):  # for longcat-image model, transformer_blocks is enough
                processors[name] = FaQAttnProcessor(
                    hidden_size=self.transformer.num_attention_heads * self.transformer.attention_head_dim,
                    cross_attention_dim=self.transformer.config.joint_attention_dim,
                ).to(self.transformer.device, dtype=self.transformer.dtype)
        ...
        def fn_recursive_attn_processor(name: str, module: torch.nn.Module, processor):
            if hasattr(module, "set_processor"):
                if not isinstance(processor, dict):
                    ...
                    module.set_processor(processor[(f"{name}.processor")])
        ...
        # step3: init image proj.
        from ..models.resampler import Resampler
        self.faq_proj_model = Resampler(
            dim=self.transformer.config.joint_attention_dim,
            depth=4,
            dim_head=64,
            heads=16,
            num_queries=16,
            embedding_dim=1152,  # siglip feature dim
            output_dim=self.transformer.config.joint_attention_dim,
            ff_mult=4,
        ).to(self.transformer.device, dtype=self.transformer.dtype)

        # step3: load ckpt
        state_dict = torch.load(quench_model_path, map_location="cpu", weights_only=True)
        self.faq_proj_model.load_state_dict(state_dict['faq_proj_model'], strict=True)
        faq_processors_layers = torch.nn.ModuleList([processor for processor in processors.values() if isinstance(processor, FaQAttnProcessor)])
        faq_processors_layers.load_state_dict(state_dict['faq_processors_layers'], strict=True)
        
        self.processors = processors
        print('prepare for quench done !!!')
    ...
    def __call__(
        ...
        if adapter_feature is not None:
            # set scale
            assert faq_scale is not None
            for attn_processor in self.processors.values():
                if isinstance(attn_processor, FaQAttnProcessor):
                    attn_processor.scale = faq_scale
            # projected adapter feature
            if self.do_classifier_free_guidance:
                negative_adapter_feature = torch.zeros_like(adapter_feature)
                negative_adapter_feature = self.faq_proj_model(negative_adapter_feature)
            adapter_feature = self.faq_proj_model(adapter_feature)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                self._joint_attention_kwargs['adapter_feature'] = adapter_feature
                ...
                if self.do_classifier_free_guidance:
                    with self.transformer.cache_context("uncond"):
                        self._joint_attention_kwargs['adapter_feature'] = negative_adapter_feature
